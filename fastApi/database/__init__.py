...
from utills.constants import DatabaseQueryStringSelect
from utills.constants import DatabaseTableCreationQueries
...
class Database:
    ...
    def insert(self, table_name, data: dict, check=True, columns: list = None):
        ...
        if len(result) == 0:
            ...
            self.__cursor.execute(
                insert_string.format(table_name, ", ".join(tuple(data.keys())), value)
            )
    ...
    def update(self, table_name: str, data: dict, condition: dict):
        ...
        condition_string = " and ".join(
            [f"{key} = {condition[key]}" for key in list(condition.keys())]
        )
        query_string = "UPDATE {} SET {} WHERE {};"
        self.__cursor.execute(
            query_string.format(table_name, update_string, condition_string)
        )
